...
interface IERC20 {
    ...
    function checkContribution(address account) external view returns (uint256);
...
contract IDO is ReentrancyGuard, Context, Ownable {
    ...
    constructor (uint256 rate, uint256 minStake, uint256 _whitelistcap , address wallet,IERC20 ido, IERC20 stakeContract, IERC20 token)  {
  ...
  /*  function withdrawTokens(address _address) public {
         ...
         IERC20 tokenBEP2 = _ido;
         tokenBEP.transfer(_address, tokenBEP.checkContribution(_address);
         _contributions[msg.sender] = 0;
    } */

    function withdrawTokens() public {
        require (claimed[msg.sender] == false);
        _ptokens = IERC20(_ido).checkContribution(msg.sender);
